...
- 私钥和对称密钥必须由受信任的密钥管理系统生成、存储和轮换，不应写入日志、命令历史或错误信息；CLI 传入 `--private-key` 会落入 shell 历史与进程列表，生产环境优先使用文件或环境变量读取。
- SM2 临时标量和密钥使用 Erlang `:crypto.strong_rand_bytes/1`。部署方仍需保证操作系统随机源正常。
- 纯 Elixir 大整数和曲线运算没有声明为常量时间。对本地攻击者、共享主机或高价值密钥场景，不应假定可抵抗缓存、调度和计时侧信道。热点剖析见 `bench/sm2_hotspot_analysis.md`（`mod/2` 占 55% OWN）。
- `secure_compare` 只减少摘要比较中的早退差异，不能使整个 SM2 解密路径成为常量时间。
- 解密端应限制输入大小（例如 `max_ciphertext = 65+32+max_plaintext`）、调用频率和错误可见性，防止资源消耗与错误预言机被协议放大；SM4 `decrypt` 的 `:invalid_padding` 与 `:invalid_block_size` 可区分，若直接暴露给远端则构成 padding oracle，建议网关层统一映射为 `:decryption_failed`。
- 本项目自审查报告见 `SECURITY_REVIEW.md`（2026-08-19），仍未替代第三方审计。
