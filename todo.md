...
当前 P0/P1/P2 基线实现与自动化验证均已完成（2026-08-19 自审查与热点分析已归档，见 `SECURITY_REVIEW.md` / `bench/sm2_hotspot_analysis.md`）；后续工作包括第三方审计等外部依赖，以及下方未勾选的高风险性能优化候选项，已完成项保留在本文中作为验收记录。
...
以下项目不属于当前已完成的 P2 基线，必须在标准兼容性和侧信道边界明确后再实施：

- [ ] 重新执行 `eprof`/`fprof`，在固定硬件、OTP/Elixir、调度器和输入规模下记录 SM2 热点，确认 `mod/2` 优化后的新瓶颈。
- [ ] 评估固定窗口或 wNAF 标量乘法，比较生成、签名、验签和加解密收益；必须补充窗口查表的缓存侧信道风险评估，不能只按吞吐量验收。
- [ ] 评估 Montgomery 域乘法与恒定时间模逆，明确是否以侧信道加固为目标；实现前先确定中间值范围、转换边界、错误语义和公开测试向量。
- [ ] 评估批量验签的 Montgomery batch inversion，明确批量 API 的失败项定位、内存上限和调用方是否接受批量语义。
- [ ] 为 SM2 性能建立可重复的基准门禁：固定运行环境或专用 runner，至少两轮独立样本，使用 median 与置信区间，避免将单次 wall-clock 作为发布阻断依据。

后续性能优化的完成条件：

- 保持 GB/T 测试向量、负面测试和 OpenSSL 双向互操作通过。
- 同机同参 median 至少改善 10%，且 max 不恶化超过 15%；结果必须附带提交号和运行环境。
- 若引入窗口化、Montgomery 或其他时序敏感实现，必须同步更新 `SECURITY.md` 和安全审查报告。
- 第三方安全审计前，不得以性能优化结果宣称纯 Elixir 曲线运算具备恒定时间安全性。
